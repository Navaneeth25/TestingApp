...
app.layout = html.Div(children =[
                    ...
                    dbc.Col(dcc.Graph(id = 'fig1',style={'height': '40vh'},figure =fig1),width=4),
                     dbc.Col(dcc.Graph(id='fig2',style={'height':'40vh'},figure=fig2),width=6) 
                    ]),
                    dbc.Row([
                    dbc.Col(width=2),
                    dbc.Col(dcc.Graph(id = 'fig4',style={'float': 'left', 'width': '100%'},figure = fig4),width=5, style={'height': '45vh'}),
                    dbc.Col(dcc.Graph(id='fig5',figure=fig5,style={'height':'43vh'}),width=5,style={'height':'45vh'})
                           ])
                    ])
                           
                    
@app.callback(Output('fig1', 'figure'),Output('fig2','figure'),Output('fig4','figure'),Output('fig5','figure'),[Input('Continent', 'value'),Input('Data Input','value'),Input('Policy','value')])
def updatefig(g,d,m):
    if g=='world':
        df=df4.copy()
    else:
    ...
    if g=="world" and g!="oceania" and d=="Confirmed Cases":
        ...
        fig5.update_layout(title_text= d +" of " + g + " from 1st MAR 2020 to 20 MAY 2020",title_x=0.3)
        fig2=px.sunburst(df, color='StringencyIndex', values=d,path=['Continent_Name','CountryName'],hover_name='Continent_Name')
        fig2.update_layout(title_text= d +" for top 5 countries",title_x=0.3)
        fig4 = px.treemap(df, path=[px.Constant('world'), 'Continent_Name','CountryName',], values=d,
                  color='StringencyIndex', hover_data=['CountryName'])
        fig4.update_layout(title_text= d +" for top 5 countries",title_x=0.3)
        if d=='ConfirmedCases':
            fig1=px.line(df3,x='Date',y=d,color='CountryName',log_y=True)
            fig1.update_layout(title_text= d +" for top 5 countries",title_x=0.3)
        else:
        ...
        fig5.update_geos(center=dict(lon=150, lat=-25), projection_rotation=dict(lon=0, lat=0, roll=0), scope='world')
        fig5.update_geos(lataxis_range=[-50, 10], lonaxis_range=[95, 180])
        fig5.update_layout(title_text= m + " of " + g + " from 1st MAR 2020 to 20 MAY 2020",title_x=0.3)
        ...
        fig1.update_layout(title_text= m +" for top 5 countries",title_x=0.3)
        if m=="School closing":
